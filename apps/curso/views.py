...
def asistencia_list(request):
    a = request.user.id
    listaasistencia = Curso.objects.raw(
        'SELECT curso.nombre,  max(horario."id") as id FROM horario INNER JOIN curso ON horario.curso_id = curso."id" INNER JOIN aula ON curso.aula_id = aula."id" INNER JOIN "public".docente ON "public".horario.docente_id = "public".docente.user_ptr_id INNER JOIN "public".auth_user ON "public".docente.user_ptr_id = "public".auth_user."id" WHERE docente.user_ptr_id =%s GROUP BY curso.nombre',
        [a])
